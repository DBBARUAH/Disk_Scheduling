...
while(1)
    {
        ...
        int i_FCFS,no_of_requests = 1000,initial_head_FCFS,initial_head;
        ...
        scanf("%d",&initial_head_FCFS);
        initial_head = initial_head_FCFS;
        ...
        for(i_FCFS=0;i_FCFS<no_of_requests;i_FCFS++)
        {
            ...
            seek_time_FCFS += abs(request[i_FCFS] - initial_head_FCFS);
            initial_head_FCFS = request[i_FCFS];
        }
        printf("\n\n\nTotal Head movement required for FCFS: %d\n\n\n", seek_time_FCFS);
        
    int i_SCAN,limit = 5000,j_SCAN,choice_SCAN,previous_head_SCAN,initial_head_SCAN;
    ...
    {
        req[i_SCAN].request_track_number = request[i_SCAN];
        //scanf("%d",&req[i].request_track_number);
        req[i_SCAN].visited = false;
    }
        initial_head_SCAN = initial_head;
    //printf("Enter initial position of R/W head: ");
    //scanf("%d",&initial_head);
    previous_head_SCAN = initial_head_SCAN;
    ...
    if(previous_head_SCAN - initial_head_SCAN > 0 )
    {
        choice_SCAN = 2;
    }
    else
    ...
    if(choice_SCAN == 1)
    {
        for(i_SCAN=initial_head_SCAN;i_SCAN<limit;i_SCAN++)
        {
            for(j_SCAN=0;j_SCAN<no_of_requests;j_SCAN++)
            {
                if(req[j_SCAN].request_track_number == i_SCAN && req[j_SCAN].visited == false)
                {
                    ...
                    seek_time_SCAN += abs(req[j_SCAN].request_track_number - initial_head_SCAN);
                    initial_head_SCAN = req[j_SCAN].request_track_number;
                }
            }
        }
        printf("%d -> ", limit-1);
        seek_time_SCAN += abs(limit-1 - initial_head_SCAN);
        initial_head_SCAN = limit-1;
        for(i_SCAN=initial_head_SCAN;i_SCAN>=0;i_SCAN--)
        {
            for(j_SCAN=0;j_SCAN<no_of_requests;j_SCAN++)
            {
                if(req[j_SCAN].request_track_number == i_SCAN && req[j_SCAN].visited == false)
        ...
        seek_time_SCAN += abs(initial_head_SCAN - 0);
        printf("0 \n");
    }
    else if(choice_SCAN == 2)
    {
        ...
        printf("%d -> ", 0);
        seek_time_SCAN += abs(0 - initial_head_SCAN);
        initial_head_SCAN = 0;
        for(i_SCAN=initial_head_SCAN;i_SCAN<limit;i_SCAN++)
        {
            for(j_SCAN=0;j_SCAN<no_of_requests;j_SCAN++)
...
//
    int i_C_SCAN,j_C_SCAN,choice_C_SCAN,previous_head_C_SCAN, initial_head_C_SCAN;
        initial_head_C_SCAN = initial_head;
    //printf("Enter the number of requests: ");
    //scanf("%d",&no_of_requests);
    ...
    for (i_C_SCAN = 0; i_C_SCAN < no_of_requests; ++i_C_SCAN)
    {
        ...
        req_C_SCAN[i_C_SCAN].request_track_number = request[i_C_SCAN];
    ...
    //printf("Enter the cylinder size: ");
    //scanf("%d",&limit);
    previous_head_C_SCAN = initial_head_C_SCAN;
    if(previous_head_C_SCAN - initial_head_C_SCAN > 0 )
    {
        choice_C_SCAN = 2;
    }
    else
    ...
    printf("\n******C_SCAN******\n\n%d -> ",initial_head_C_SCAN );
    int cp_initial_head = initial_head_C_SCAN;
    if(choice_C_SCAN == 1)
    {
        for(i_C_SCAN=initial_head_C_SCAN;i_C_SCAN<limit;i_C_SCAN++)
        {
            for(j_C_SCAN=0;j_C_SCAN<no_of_requests;j_C_SCAN++)
            {
                if(req_C_SCAN[j_C_SCAN].request_track_number == i_C_SCAN && req_C_SCAN[j_C_SCAN].visited == false)
                {
                    ...
                    seek_time_C_SCAN += abs(req_C_SCAN[j_C_SCAN].request_track_number - initial_head_C_SCAN);
                    initial_head_C_SCAN = req_C_SCAN[j_C_SCAN].request_track_number;
                }
            }
        }
        printf("%d -> \n", limit-1);
        seek_time_C_SCAN += abs(limit-1 - initial_head_C_SCAN);
        initial_head_C_SCAN = 0;
    ...
    {
        for(i_C_SCAN=initial_head_C_SCAN;i_C_SCAN>=0;i_C_SCAN--)
        {
            for(j_C_SCAN=0;j_C_SCAN<no_of_requests;j_C_SCAN++)
            {
                if(req_C_SCAN[j_C_SCAN].request_track_number == i_C_SCAN && req_C_SCAN[j_C_SCAN].visited == false)
        ...
        printf("%d -> ", 0 );
        seek_time_C_SCAN += abs(initial_head_C_SCAN - 0);
        initial_head_C_SCAN = limit-1;
